...
class TradeappApplicationTests {
	...
	@Test
	void testTradeStore_successful() {
		ResponseEntity responseEntity = tradeController.tradeValidateStore(createMockTrade("T1",1,LocalDate.now()));
	...
	@Test
	void testTradeStoreWhenMaturityDatePast() {
		try {
			LocalDate localDate = getLocalDate(2015, 05, 21);
			ResponseEntity responseEntity = tradeController.tradeValidateStore(createMockTrade("T2", 1, localDate));
	...
	@Test
	void testTradeStoreWhenOldVersion() {
		// step-1 create trade
		ResponseEntity responseEntity = tradeController.tradeValidateStore(createMockTrade("T1",2,LocalDate.now()));
		Assertions.assertEquals(ResponseEntity.status(HttpStatus.OK).build(),responseEntity);
		List<Trade> tradeList =tradeController.findAllTrades();
		Assertions.assertEquals(1, tradeList.size());
		Assertions.assertEquals("T1",tradeList.get(0).getTradeId());
		...
		try {
			ResponseEntity responseEntity1 = tradeController.tradeValidateStore(createMockTrade("T1", 1, LocalDate.now()));
	...
	@Test
	void testTradeStoreWhenSameVersionTrade(){
		ResponseEntity responseEntity = tradeController.tradeValidateStore(createMockTrade("T1",2,LocalDate.now()));
		Assertions.assertEquals(ResponseEntity.status(HttpStatus.OK).build(),responseEntity);
		List<Trade> tradeList =tradeController.findAllTrades();
		Assertions.assertEquals(1, tradeList.size());
		...
		Trade trade2 = createMockTrade("T1",2,LocalDate.now());
		...
		Trade trade3 = createMockTrade("T1",2,LocalDate.now());
	...
	private Trade createMockTrade(String tradeId,int version,LocalDate  maturityDate){
