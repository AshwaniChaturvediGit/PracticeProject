package com.springboot.dbank.tradeapp;
...
import com.springboot.dbank.tradeapp.rest.TradeController;


public class TradeAppTest extends TradeappApplicationTests {

	@Autowired
	private TradeController tradeController;

	@Test
	public void testTradeValidateAndStore_successful() {
		ResponseEntity<String> responseEntity = tradeController.tradeValidateStore(createTrade("T1",1,LocalDate.now()));
	...
	@Test
	public void testTradeValidateAndStoreWhenMaturityDatePast() {
		try {
			LocalDate localDate = getLocalDate(2015, 05, 21);
			ResponseEntity<String> responseEntity = tradeController.tradeValidateStore(createTrade("T2", 1, localDate));
		}catch (InvalidTradeStoreException ie) {
	...
	@Test
	public void testTradeValidateAndStoreWhenOldVersion() {
		// step-1 create trade
		ResponseEntity<String> responseEntity = tradeController.tradeValidateStore(createTrade("T1",2,LocalDate.now()));
		Assertions.assertEquals(ResponseEntity.status(HttpStatus.OK).build(),responseEntity);
		List<Trade> tradeList =tradeController.findAllTrades();
		Assertions.assertEquals(1, tradeList.size());
		Assertions.assertEquals("T1",tradeList.get(0).getTradeId());
		...
		try {
			ResponseEntity<String> responseEntity1 = tradeController.tradeValidateStore(createTrade("T1", 1, LocalDate.now()));


		}catch (InvalidTradeStoreException e){
	...
	@Test
	public void testTradeValidateAndStoreWhenSameVersionTrade(){
		ResponseEntity<String> responseEntity = tradeController.tradeValidateStore(createTrade("T1",2,LocalDate.now()));
		Assertions.assertEquals(ResponseEntity.status(HttpStatus.OK).build(),responseEntity);
		List<Trade> tradeList =tradeController.findAllTrades();
		Assertions.assertEquals(1, tradeList.size());
		...
		ResponseEntity<String> responseEntity2 = tradeController.tradeValidateStore(trade2);
		...
		ResponseEntity<String> responseEntity3 = tradeController.tradeValidateStore(trade3);
